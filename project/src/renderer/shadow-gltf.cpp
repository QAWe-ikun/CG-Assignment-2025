...
namespace renderer
{
	...
	void Shadow_gltf::Drawdata::CSM_level_data::append(const gltf::Drawdata& drawdata) noexcept
	{
	...
	glm::mat4 Shadow_gltf::Drawdata::CSM_level_data::get_vp_matrix() const noexcept
	{
	...
	{
		return pipeline::Shadow_gltf::create(device)
			.transform([](pipeline::Shadow_gltf&& shadow_pipeline) noexcept {
				return Shadow_gltf{std::move(shadow_pipeline)};
			})
