...
{
    vec4 shadow_clip_space_pos = param.shadow_VP_levels[level] * vec4(world_pos, 1.0);
    shadow_clip_space_pos /= shadow_clip_space_pos.w;
    vec2 shadow_uv = ndc_to_uv(shadow_clip_space_pos.xy);

    // if (abs(shadow_clip_space_pos.x) < 1.0 && abs(shadow_clip_space_pos.y) < 1.0 && shadow_clip_space_pos.z < 1.0)
...
void main()
{
    ...
    vec2 ndc = (vec2(gl_FragCoord.xy) + 0.5) / vec2(texture_size) * vec2(2.0, -2.0) + vec2(-1.0, 1.0);

    /* Fetch Texture */

    uvec2 light_info_tex_sample = texelFetch(light_info_tex, fragcoord, 0).rg;
    float depth = texelFetch(depth_tex, fragcoord, 0).r;

    /* Get Albedo */

    vec3 albedo = texelFetch(albedo_tex, fragcoord, 0).rgb;
    ...
    vec4 world_pos = param.VP_inv * clip_space_pos;
    world_pos /= world_pos.w;

    vec3 target_to_eye = normalize(param.eye_position - world_pos.xyz);
    ...
    /* Output */
    out_light_buffer = vec4(pbr_color * shadow_mult, 0.0);
}
