...
#include "include/ndc-uv-conv.glsl"
...
vec2 uv = (vec2(fragcoord) + vec2(0.5)) / vec2(ao_render_size);
...
vec3 get_view_normal()
{
    ...
    vec4 view_normal = params.view_mat * vec4(world_normal, 0.0);
...
void main()
{
    ...
    const vec3 world_pos = homo_transform(params.camera_mat_inv, vec3(ndc, depth));
    const vec3 view_pos = homo_transform(params.proj_mat_inv, vec3(ndc, depth));
    const vec3 view_normal = get_view_normal();

    const vec3 offseted_world_pos = homo_transform(
            params.camera_mat_inv,
            vec3(ndc + vec2(params.radius) / vec2(depth_render_size), depth)
        );
    ...
    {
        const float jittered_phi = phi + noise_function(uv * vec2(156.213, 481.53), params.random_seed + phi);
        const vec2 sample_direction = vec2(cos(jittered_phi), sin(jittered_phi));
        const vec2 sample_step = sample_direction * params.radius / depth_render_size / SAMPLE_COUNT;
        ...
        {
            sample_ndc += sample_step * mix(0.5, 1.5, noise_function(sample_ndc * vec2(152.12, 843.2), params.random_seed));
            ...
            const vec3 sample_view_pos = homo_transform(params.proj_mat_inv, vec3(sample_ndc, sample_depth));
    ...
    vec4 prev_homo = params.prev_camera_mat * vec4(world_pos, 1.0);
    if (prev_homo.w > 0.0)
    {
        ...
        if (all(lessThanEqual(abs(prev_ndc.xy), vec2(1.0))))
        {
            ...
            if (distance(prev_depth, prev_ndc.z) / prev_depth < 0.00001)
            {
                ...
                occlusion_sum = mix(prev_ao, occlusion_sum, params.blend_alpha);
            }
        }
    }
