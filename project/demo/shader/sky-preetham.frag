...
vec3 preetham_sky_rgb(vec3 v, vec3 sun_dir)
{
    ...
    vec3 R_xyY = params.p_Z * perez(cos_theta, gamma, cos_gamma, params.p_A, params.p_B, params.p_C, params.p_D, params.p_E);
...
void main()
{
    ...
    vec4 view_dir_h = params.camera_mat_inv * vec4(ndc.xy, 0.5, 1.0);
    vec3 view_dir = normalize(view_dir_h.xyz / view_dir_h.w - params.eye_position);

    out_color = vec4(preetham_sky_rgb(view_dir, params.sun_dir) * params.sun_intensity, 0.0);
}
