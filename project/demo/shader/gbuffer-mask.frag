...
layout(location = 4) in vec4 debug_pos;
...
void main()
{
    ...
    if (albedo_tex_sample.a < material.alpha_cutoff) discard;

    /* Albedo */

    vec3 out_color = pow(
            albedo_tex_sample.rgb * material.base_color_factor, // Get color and apply factor
    ...
    lighting.metalness = metalness_roughness_tex_sample.x * material.metallic_factor;
    lighting.roughness = metalness_roughness_tex_sample.y * material.roughness_factor;
    lighting.occlusion = 1 - (1 - occlusion_tex_sample) * material.occlusion_strength;

    out_light_info = pack_gbuffer_lighting(lighting);

    /* Emissive */

    vec3 emissive = texture(emissive_tex, in_uv).rgb * material.emissive_factor;
    out_light_buffer = vec4(emissive, 1.0);
}
