...
namespace gltf
{
	...
	// Stores indices to textures used by a material
	...
	struct Material_cache
	{
		...
		struct Ref
		{
			std::span<const Material_bind> material_binds;
			std::reference_wrapper<const Material_bind> default_material_bind;

			// Get material bind for a drawcall
			Material_bind get_material_bind(std::optional<uint32_t> material_index) const noexcept;
		};
		...
		std::vector<Material_bind> material_bind_cache;  // Cache of material bindings
		Material_bind default_material_bind;             // Default material binding

		// Get reference to the material cache
		Ref ref() const noexcept;
	};

	// Reference to material cache
	...
	class Material_list
	{
	  ...
	  private:
		...
		///
		/// @brief Get texture sampler binding for normal map, or default if not found
		///
		/// @param texture_index Input texture index
		/// @return Texture sampler binding on success, or nullopt on failure
		///
		std::optional<SDL_GPUTextureSamplerBinding> get_texture_sampler_binding_normal(
			std::optional<uint32_t> texture_index
		) const noexcept;
		...
		///
		std::optional<Material_bind> gen_binding_info(std::optional<uint32_t> material_index) const noexcept;
